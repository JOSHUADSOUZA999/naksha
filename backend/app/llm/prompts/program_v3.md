...
- `hall` **open** to `dining` — one living and dining space with no wall between, the
  way most new Indian houses are planned. Use **connected** (a door) only when the brief
  asks for a separate or formal dining room.
...
**`near` is how a brief's requirement becomes something the solver must honour.** It
means a short walk apart — about ten metres, the walk from the front door to the living
room and a few steps more — without adding a door between them. When the brief says a room should be near another, write
the edge, `hard: true`, and say why in the room's `why`:

- "elderly parents … near the entrance" → the parents' bedroom **near** `foyer`
- "study near the living room" → `study` **near** `hall`
- "guest room close to the entrance" → the guest room **near** `foyer`

Only between rooms a person walks between: never a car bay, which is entered from the
street, not through the house.

A sentence in `why` alone changes nothing: nothing downstream reads it. If the brief asks
for a room to be near something, the edge is the only way the plan will show it.
